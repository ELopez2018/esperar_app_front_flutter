...
class PlateVehicle extends StatelessWidget {
  const PlateVehicle(
      {Key? key, required this.background, this.plate, this.number, this.width, this.fontSize})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: width ?? (plate == null && number == null
          ? 100
          : plate != null
              ? 100
              : 60),
        ...
        painter: GenerateVehiclePlate(
            text: plate == null && number == null
                ...
                : plate != null
                    ? plate!
                    : number.toString(),
            color: background, fontSize: fontSize),
      ),
    );
  }
}
...
class GenerateVehiclePlate extends CustomPainter {
  GenerateVehiclePlate({required this.text, required this.color, this.fontSize});
  final String text;
  final Color color;
  final double? fontSize;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final TextPainter textPainter = TextPainter(
      text: TextSpan(
        text: text,
        style:  TextStyle(
            color: Colors.black, fontSize:  fontSize ?? 16.0, fontWeight: FontWeight.w700),
      ),
